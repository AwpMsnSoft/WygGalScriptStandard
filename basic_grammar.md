...
- 本标准草案中的指代不使用双引号，文本，补充说明使用圆括号（），分行说明使用分号；。
- 语法功能集和指令集可使用大写字母缩写代称，其中语法功能集缩写的第一个字母为G。
 
## wgs的文件名:

- wgs脚本的文件名以.wgs结尾，即文件名为*.wgs，扩展名为.wgs。
...
- 为保证脚本能够被解释器正常识别，由引擎对脚本进行校验、编码转换后发送给解释器。

## 脚本文件读取:

- 脚本可以在运行前读取，也可在运行过程中读取，由引擎负责；
- 运行过程中，不应修改相关脚本文件的内容，否则会导致不可预期的后果；
- 脚本文件无法读取属于关键错误。
...
- 使用十进制定点数存储和计算，精确到小数点后6位，舍去更高的小数位数，整数部分不超过18位时不能溢出；
- 使用数类型，不应使其整数部分超过18位，否则在一些环境中会得到错误的结果。对整数部分超过18位的数，应使用文本类型或分节表示和计算；
...
- 在用一对双引号表示的文本中，不应出现\（反斜杠）和^（Caret），因为它们在一些环境中无法正常解析。
...
- 文本比较的不区分大小写对英文生效，对其他语种的字母是否生效取决于系统和当前地理位置，应当引起注意；
...
## 常量：
- 常量是恒定不变的数据单元，包括默认常量、布尔常量、数常量、文本常量、数组，例如Default、True、123.4、"常量"、{3,4,5}。

## 常量标识符（GCONST）：
- 常量标识符用于指代表示常量，在形式上具有与变量完全相同的表示方法，例如：\$CONST、[]CONSTARR、[0]CONSTARR ；文本保留常量是一种常量标识符；
- 常量标识符一经定义，无法修改，也不允许取消定义，否则是关键错误（除非使用.reset指令）；
...
## 执行指令（GCMD）： 
  ...
  ##若指令允许，则参数列表尾部的若干个Default可以省略，例如
...
- 指令、变量、数组、常量标识符的命名全部使用半角字符，不区分大小写，不允许重名；
- 用户定义变量、数组、常量标识符的命名不允许为指令名称或保留字，非标准指令名称不允许为保留字，否则是关键错误。

## 语法逃逸（GEXC）：
- 为提高wgs编写效率，兼顾编写习惯，部分指令的格式可不符合wgs语法，例如.message指令和.chain指令。