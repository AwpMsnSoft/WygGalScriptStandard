...
- .loop和.endloop的用法：
    ```
    ##.loop为循环的开始，.endloop为循环的结尾，循环即反复执行##repeat_do_things处的语句，
...
- 指令说明：
    ...
    ```
        ...
        .chain "WgsNoExten.wgs"
    ```
    - WgsFileName允许包含目录，允许使用相对路径，不建议使用绝对路径，**如果包含空格，或可引发歧义，或包含目录，则文本常量必须加双引号，否则无法实现预期目标**。
    - WgsFileName指代的文件不一定为硬盘上的文本文件，它可能是某个文件的一部分，或加密的文本文件，或网络请求的返回数据，或内存空间中的数据，或随机生成的脚本。
    - WgsFileName允许指代当前正在使用的wgs文件，相当于回当前wgs的第一行，这可能因变量的重定义而引发关键错误，应在变量失去作用后解除定义，或者一开始就使用.redef而不是.defvar指令。
...
- 指令说明：
    ...
    - 使用.call指令，跳转到先前离开的若干wgs文件可能性更大，更容易因变量的重定义而引发关键错误，应在变量失去作用后解除定义，或者一开始就使用.redef而不是.defvar指令；
...
- 指令说明：
    - 解释器若支持缓存功能，则清除缓存的wgs文件（不清除正在使用和调用堆栈中记录的wgs文件缓存），否则什么也不做；
    - 本指令具有两个作用，一是减少内存消耗，二是用于接收解释器外随机生成的wgs文件。重新请求并接收被清空缓存的wgs需要额外时间。
-------- 
## 正常退出指令 (PEXIT指令)
- 使用正常退出指令退出或返回，不属于异常退出。
### **.exit**  
> 概述：退出当前执行的wgs文件。 
- 参数列表
    |参数名称|默认值|文本|数|布尔|数组|输入|输出|表达式空格|
...
- 指令说明：
    - 退出当前执行的wgs文件，并且：如果先前通过.call指令到达此处，则回到原先wgs的.call指令之后，否则使解释器进入待机状态；
    - 不清除已定义的全局变量、数组、常量标识符。
--------
### **.finish**  
> 概述：退出当前执行的wgs文件，清空调用堆栈，并立即使解释器进入待机状态。 
- 参数列表
    |参数名称|默认值|文本|数|布尔|数组|输入|输出|表达式空格|
    |:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
    |无|||||||||
...
- 指令说明：
    - 退出当前执行的wgs文件，并且无论是否通过.call指令到达此处，均使解释器进入待机状态，调用堆栈也一并清空；
    - 不清除已定义的全局变量、数组、常量标识符。
--------
### **.reset**  
> 概述：复位指令，使解释器回到最初的待机状态。 
- 参数列表
    |参数名称|默认值|文本|数|布尔|数组|输入|输出|表达式空格|
    |:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
    |无|||||||||
...
- 指令说明：
    - 退出当前执行的wgs文件，清空调用堆栈，解除所有非保留的全局变量、数组、常量标识符的定义，使解释器回到最初状态并待机。
--------
